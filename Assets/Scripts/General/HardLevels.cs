...
public class HardLevels : MonoBehaviour {
	...
	/* Motor principal de los niveles de dificultad jodida */
	...
	void Update () {
		...
		if (vars.d_hard && vars.level == 5 && vars.street) {
			...
			if (!over.end) {
				...
				//Controlamos la recarga del arma
		...
		}else if (vars.d_hard && vars.level == 8 && vars.street) {
		...
		}else if (vars.d_hard && vars.level == 11 && vars.street){
			...
			if (!over.end) {
				...
				//se agote, pasaremos de nivel o no.
				if (n_cats >= maxCatsKills && tempoMuertes.timeLeft >= 0) {
					CarLife car = GameObject.Find ("CarPerfect").GetComponent<CarLife> ();
					if (car.resistencia <= 0) {
						GameObject nextLevel = GameObject.Find ("Next Level");
						Text texto = nextLevel.GetComponent<Text> ();
						noRecargar.noPressR = true;
						if (timeWait > 0) {
					...
					}else if (car.resistencia > 0) {
						GameObject nextLevel = GameObject.Find ("Next Level");
						Text texto = nextLevel.GetComponent<Text> ();
						if (timeWait > 0) {
							texto.enabled = true;
							timeWait -= Time.deltaTime;
							texto.text = "VICTORY!! Though... You Can Improve ";
